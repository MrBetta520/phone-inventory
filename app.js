...
function render(){const stock=currentStock();const today=phones.filter(p=>dateKey(p.receivedAt)===dateKey()).length;$("#stockCount").textContent=totalCurrent();$("#todayCount").textContent=today;$("#shippedCount").textContent=phones.filter(p=>p.status===STATUS.SHIPPED).length;$("#modelCount").textContent=stock.length;renderStats(stock);renderRecent();renderPhoneTables();renderCustomerStock(stock);renderAdjustments();renderProducts()}
...
function submitAdjustment(e){e.preventDefault();const item={id:`adj_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,customer:$("#adjustCustomer").value,model:$("#adjustModel").value.trim(),color:$("#adjustColor").value.trim(),capacity:$("#adjustCapacity").value.trim(),type:$("#adjustType").value,quantity:Number($("#adjustQuantity").value),note:$("#adjustNote").value.trim(),createdAt:new Date().toISOString()};if(!Number.isInteger(item.quantity)||item.quantity<1)return;if(item.type==="decrease"&&quantityFor(item)<item.quantity){$("#adjustMessage").textContent=`不能减少 ${item.quantity} 台；该客户此配置当前只有 ${quantityFor(item)} 台。`;return}adjustments.unshift(item);e.currentTarget.reset();$("#adjustMessage").textContent="✅ 库存调整已保存，并已同步到客户库存总表。";save()}
...
function bind(){$$('nav button').forEach(b=>b.onclick=()=>showView(b.dataset.view));$("#tracking").onkeydown=e=>{if(e.key==="Enter"){e.preventDefault();$("#imei").focus()}};$("#imei").onkeydown=e=>{if(e.key==="Enter"){e.preventDefault();$("#upc").focus()}};$("#upc").onkeydown=e=>{if(e.key==="Enter"){e.preventDefault();receivePhone()}};$("#receiveBtn").onclick=receivePhone;$("#adjustmentForm").onsubmit=submitAdjustment;$("#productForm").onsubmit=submitProduct;$("#csvImport").onchange=importCsv;$("#stockSearch").oninput=renderPhoneTables;$("#shipSearch").oninput=renderPhoneTables;$("#stockCustomerFilter").onchange=()=>renderCustomerStock(currentStock());$("#exportAll").onclick=exportAll;$("#exportCustomerToday").onclick=exportToday;$("#exportCustomerStock").onclick=exportStock;$("#receiveCustomer").onchange=()=>{$("#message").textContent=$("#receiveCustomer").value?"请扫描快递单号":"请先选择客户"}}
