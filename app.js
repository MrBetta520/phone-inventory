...
const VIEW_CONTENT={
 ...
 ship:["出库管理","扫描 IMEI 自动匹配手机并减少库存"],products:["UPC 型号库","维护 UPC 与手机型号的对应关系"]
};
...
function shippedToday(customer){return phones.filter(p=>p.customer===customer&&p.status===STATUS.SHIPPED&&dateKey(p.shippedAt)===dateKey())}
...
async function shipPhone(imei,askConfirm=true){
 if(!requireDataFile())return false;
 imei=String(imei||"").trim();const message=$("#shipMessage");
 if(!imei){message.textContent="⚠️ 请先扫描 IMEI";return false}
 const index=phones.findIndex(p=>String(p.imei)===imei);
 if(index<0){message.textContent="⚠️ 没有找到这个 IMEI；期初库存没有逐台 IMEI，不能通过此处出库";return false}
 const phone=phones[index];
 if(phone.status===STATUS.SHIPPED){message.textContent=`⚠️ 这个 IMEI 已经出库：${phone.customer} · ${phone.model} · ${formatDate(phone.shippedAt)}`;return false}
 if(askConfirm&&!confirm(`确认出库？\n${phone.customer} · ${phone.model} · ${phone.color} · ${phone.capacity}\nIMEI：${phone.imei}`))return false;
 const previous=phone;phones[index]={...phone,status:STATUS.SHIPPED,shippedAt:new Date().toISOString()};
 if(await save()){message.textContent=`✅ 出库成功：${phone.customer} · ${phone.model} · ${phone.color} · ${phone.capacity}`;$("#shipImei").value="";$("#shipImei").focus();return true}
 phones[index]=previous;render();message.textContent="⚠️ Excel 保存失败，本次出库已撤销，请处理文件后重新扫描";return false
}
function shipScannedImei(){return shipPhone($("#shipImei").value,false)}
...
function detailRows(list){return [["客户","快递单号","IMEI","型号","颜色","容量"],...list.map(p=>[p.customer,String(p.tracking||""),String(p.imei||""),p.model,p.color,p.capacity])]}
function exportInStockDetails(){const list=phones.filter(p=>p.status===STATUS.IN_STOCK);if(!list.length){alert("当前没有扫码入库且仍在库的手机。");return}writeXlsx(detailRows(list),"全部在库明细",`全部在库手机明细_${dateKey()}.xlsx`)}
function selectedCustomer(){const c=$("#stockCustomerFilter").value;if(c==="ALL"){alert("请先在“查看客户”中选择一个客户。");return null}return c}
function exportToday(){const c=selectedCustomer();if(!c)return;const list=scannedToday(c);if(!list.length){alert(`${c} 今天还没有扫码入库记录。`);return}writeXlsx(detailRows(list),"今日入库明细",`${c}_今日入库明细_${dateKey()}.xlsx`)}
function exportShippedToday(){const c=$("#shipCustomerFilter").value;if(!c){alert("请先选择要导出的客户。");return}const list=shippedToday(c);if(!list.length){alert(`${c} 今天还没有扫码出库记录。`);return}writeXlsx(detailRows(list),"今日出库明细",`${c}_今日出库明细_${dateKey()}.xlsx`)}
...
function fillCustomers(){const options=CUSTOMERS.map(c=>`<option value="${c}">${c}</option>`).join("");$("#receiveCustomer").insertAdjacentHTML("beforeend",options);$("#adjustCustomer").insertAdjacentHTML("beforeend",options);$("#stockCustomerFilter").insertAdjacentHTML("beforeend",options);$("#shipCustomerFilter").insertAdjacentHTML("beforeend",options);$("#stockCustomerFilter").insertAdjacentHTML("beforeend",'<option value="未分配">未分配（旧数据）</option>')}
function bind(){$$('nav button').forEach(b=>b.onclick=()=>showView(b.dataset.view));$("#tracking").onkeydown=e=>{if(e.key==="Enter"){e.preventDefault();$("#imei").focus()}};$("#imei").onkeydown=e=>{if(e.key==="Enter"){e.preventDefault();$("#upc").focus()}};$("#upc").onkeydown=e=>{if(e.key==="Enter"){e.preventDefault();receivePhone()}};$("#receiveBtn").onclick=receivePhone;$("#shipImei").onkeydown=e=>{if(e.key==="Enter"){e.preventDefault();shipScannedImei()}};$("#shipScanBtn").onclick=shipScannedImei;$("#adjustmentForm").onsubmit=submitAdjustment;$("#adjustModel").onchange=refreshAdjustmentColors;$("#adjustColor").onchange=refreshAdjustmentCapacities;$("#productForm").onsubmit=submitProduct;$("#csvImport").onchange=importCsv;$("#stockSearch").oninput=renderPhoneTables;$("#shipSearch").oninput=renderPhoneTables;$("#stockCustomerFilter").onchange=()=>renderCustomerStock(currentStock());$("#exportInStockDetails").onclick=exportInStockDetails;$("#exportCustomerToday").onclick=exportToday;$("#exportCustomerStock").onclick=exportStock;$("#exportCustomerShippedToday").onclick=exportShippedToday;$("#createDataFile").onclick=createDataFile;$("#openDataFile").onclick=openDataFile;$("#receiveCustomer").onchange=()=>{$("#message").textContent=$("#receiveCustomer").value?"请扫描快递单号":"请先选择客户"}}
